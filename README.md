...
Оба меню **black** и **white** поддерживают множественное введение адресов ip-адресов. Маска адреса указывается через обратный слеш - **&laquo;/&raquo;**. Например: 192.168.0.2/32. При этом вы вполне можете указать и ipv4 и ipv6 в одной команде. Как было описано выше протокол автоматически будет поправлен в процессе работы с адресом.
